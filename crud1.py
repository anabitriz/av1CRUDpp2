...
def alterar_art(playlista):
    ...
    for a in playlista:
        ...
        if nome == nomeA:
            ...
            idad = int(input("idade do novo artista"))
            nacio = input ("nacionalidade do novo artista")
            playlista[playlista.index(a)] = nome, novoArt, idad, nacio
            return
            print (f'artista: {nomeA} não encontrado')

def alterar_alb(play):
    nomeB = input("nome do artista")
    for b in play:
        nome_album, lançam = b
        if nome_album == nomeB:
...
def alterar_mus(playmus):
    ...
    for m in playmus:
        ...
        if Nmusica == nomeM:
            ...
            return
            print (f'mudica: {nomeM} não encontrada')
